...
def jobs():
  ...
  SELECT p.id, p.title, p.description, p.country, org.name as org_name, org.description as org_description 
...
def users():
    return DB.fetch_lazy('''
  SELECT id, bio, mission, skills, country FROM users ORDER BY created_at DESC
''')


def orgs():
    return DB.fetch_lazy('''
  SELECT id, bio, description, culture, country FROM organizations ORDER BY created_at DESC
''')
